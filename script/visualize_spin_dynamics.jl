...
# 磁場の向き
arrows3d!(ax, [Point3f(0)], [Point3f(n1...)], tipradius=0.04, tiplength=0.08, shaftradius=0.007, color=:red, transparency=true, label="0-t1")
arrows3d!(ax, [Point3f(0)], [Point3f(n2...)], tipradius=0.04, tiplength=0.08, shaftradius=0.007, color=:cyan, transparency=true, label="t1-t2")
...
arrows3d!(ax, [Point3f(0)], vec_fwd, tipradius=0.04, tiplength=0.08, shaftradius=0.007, color=:orange, transparency=true)
...
arrows3d!(ax, [Point3f(0)], vec_bwd, tipradius=0.04, tiplength=0.08, shaftradius=0.007, color=:dodgerblue, transparency=true)
...
record(fig, "bloch_trajectory.gif", times; framerate = 20) do t
    ...
    # Observable更新
    push!(traj_fwd[], v_fwd); notify(traj_fwd)
    push!(traj_bwd[], v_bwd); notify(traj_bwd)
    vec_fwd[] = [v_fwd]
    vec_bwd[] = [v_bwd]
